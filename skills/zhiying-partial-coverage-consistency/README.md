# 部分生效一致性排查 · zhiying-partial-coverage-consistency

> 排查并修复部分可用状态在多处消费点不一致的缺陷：某维度缺数据被剔除，下游却仍按完整生效展示或计算。
...
| 工具 | 方式 |
| --- | --- |
| **WorkBuddy** | 推荐市场搜索「部分生效一致性排查」一键安装，或把本目录复制到 `~/.workbuddy/skills/` |
